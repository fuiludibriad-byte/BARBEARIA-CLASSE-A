...
const AdminPanel = ({ onLogout }: AdminPanelProps) => {
  ...
  const authUser = useMemo(() => {
    const saved = localStorage.getItem('classea_authenticated_barber') || sessionStorage.getItem('classea_authenticated_barber');
    if (saved) {
      try {
  ...
  const handleAddSlot = () => {
    ...
    if (slotsMode === 'weekly') {
      ...
      fetch('/api/calendar', {
        ...
        body: JSON.stringify({ type: 'add_slot', weekday: selectedWeekday, time: newSlotTime, barberId: activeSlotsBarberId })
      })
        .then(async (res) => {
  ...
  const handleDeleteSlot = (weekdayOrDate: number | string, time: string, isDate: boolean = false) => {
    if (!isDate) {
      ...
      fetch('/api/calendar', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ type: 'delete_slot', weekday: wk, time, barberId: activeSlotsBarberId })
      })
        .then(async (res) => {
          if (!res.ok) throw new Error('Erro no servidor');
          return res.json();
  ...
  const handleClearSlots = () => {
    if (slotsMode === 'weekly') {
      ...
      fetch('/api/calendar', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ type: 'clear_slots', weekday: selectedWeekday, barberId: activeSlotsBarberId })
      })
        .then(async (res) => {
          if (!res.ok) throw new Error('Erro no servidor');
          return res.json();
  ...
  const handleCopySlots = () => {
    if (slotsMode === 'weekly') {
      ...
      fetch('/api/calendar', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ type: 'copy_slots', fromWeekday: selectedWeekday, toWeekdays: targetDaysForCopy, barberId: activeSlotsBarberId })
      })
        .then(async (res) => {
          if (!res.ok) throw new Error('Erro no servidor');
          return res.json();
  ...
  const completedForDashboard = useMemo(() => {
    ...
    return completed.filter(b => (b.barberId || 'luiz') === dashboardBarberFilter);
  }, [completed, authUser, dashboardBarberFilter]);

  useEffect(() => {
    if (!activeSlotsBarberId) return;
    fetch(`/api/calendar?action=get_slots&barberId=${activeSlotsBarberId}`)
      .then(res => {
        if (!res.ok) throw new Error();
        return res.json();
      })
      .then(data => {
        if (Array.isArray(data.slots)) setWhitelistSlots(data.slots);
        if (Array.isArray(data.dateSlots)) setDateSlots(data.dateSlots);
      })
  ...
  const reload = () => {
    ...
    const fetchBarberId = authUser.role === 'owner' ? 'qualquer' : authUser.id;
    // Busca eventos em tempo real do Google Calendar API (fonte da verdade)
    fetch(`/api/calendar?t=${Date.now()}&barberId=${fetchBarberId}`, {
      headers: {
  ...
  const filteredCompleted = useMemo(() => {
    const now = new Date();
    return completedForDashboard.filter(b => {
  ...
  const acceptedCount = displayBookings.filter(b => b.status === 'accepted').length;
  ...
  const unifiedAgenda = useMemo(() => {
    ...
    const listBookings = subFilter === 'completed' ? displayCompleted : displayBookings;
  ...
  }, [displayBookings, displayBlocks, displayCompleted, subFilter]);
  ...
  const tabs: { key: TabType; label: string; icon: React.ReactNode; badge?: number }[] = [
    { key: 'bookings', label: 'Agendamentos', icon: <CalendarDays className="w-4 h-4" />, badge: displayBookings.length + displayBlocks.length },
        ...
        <div className="p-4 md:p-6 border-b border-primary/10 flex justify-between items-center bg-card/50 backdrop-blur-xl sticky top-0 z-10">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">Barbearia Classe A</p>
            </div>
          </div>
          <div className="flex gap-4">
            ...
            <button onClick={onLogout} className="flex items-center gap-2 text-muted-foreground hover:text-destructive transition-colors text-sm border-l border-primary/10 pl-4">
        ...
        <div className="p-4 md:p-8">
                        ...
                        <div className="space-y-1">
                          ...
                          <div className="flex flex-wrap gap-2">
                            <span className="inline-flex items-center gap-1.5 text-xs bg-primary/10 text-primary px-2.5 py-1 rounded-full font-medium">
                              <Sparkles className="w-3 h-3" /> {a.service}
                            </span>
                            ...
                            {authUser.role === 'owner' && (
                              <span className="inline-flex items-center gap-1.5 text-xs bg-indigo-500/10 text-indigo-400 px-2.5 py-1 rounded-full font-bold uppercase tracking-wider">
                                👤 {a.barberId === 'vitinho' ? 'Vitinho' : 'Luiz'}
                              </span>
                            )}
                          </div>
                      ...
                      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div className="space-y-1">
                          ...
                          <div className="flex flex-wrap gap-2">
                            ...
                            {authUser.role === 'owner' && (
                              <span className="inline-flex items-center gap-1.5 text-xs bg-indigo-500/10 text-indigo-400 px-2.5 py-1 rounded-full font-bold uppercase tracking-wider">
                                👤 {bl.barberId === 'vitinho' ? 'Vitinho' : 'Luiz'}
                              </span>
                            )}
                          </div>
                        </div>
          ...
          {tab === 'dashboard' && (
            <div className="space-y-8">
              {authUser.role === 'owner' && (
                <div className="flex justify-between items-center bg-card/65 backdrop-blur-sm p-4 rounded-xl border border-primary/10">
                  <p className="text-sm font-medium text-muted-foreground">Filtro por Profissional:</p>
                  <select
                    value={dashboardBarberFilter}
                    onChange={(e) => setDashboardBarberFilter(e.target.value)}
            ...
            <div className="max-w-lg mx-auto space-y-6">
              {authUser.role === 'owner' && (
                <div className="flex items-center justify-between bg-primary/5 p-4 rounded-xl border border-primary/10">
                  <span className="text-sm font-medium text-muted-foreground">Cadastrar para:</span>
                  <select
              ...
              <div className="bg-card/85 backdrop-blur-xl p-6 md:p-8 rounded-2xl border border-primary/10 shadow-[0_0_0_1px_rgba(255,255,255,0.05),0_20px_50px_-15px_rgba(0,0,0,0.5)] space-y-6">
                ...
                {authUser.role === 'owner' && (
                  <div className="flex items-center gap-3 p-4 bg-primary/5 rounded-xl border border-primary/10">
                    <span className="text-sm font-medium text-muted-foreground">Visualizando horários de:</span>
                    <select
                      value={selectedBarberTabId}
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      placeholder="Ex: Barbearia Classe A"
                      className="w-full bg-background/50 border border-primary/10 focus:border-primary/40 p-3.5 rounded-xl outline-none transition-all text-foreground text-sm placeholder:text-muted-foreground/40"
                    />
                  </div>
                  <div>
